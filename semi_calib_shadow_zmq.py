...
class Semi_calib(object):
    ...
    def run(self,dep):
        ...
        lights_path = f'testData/0909_50_zhuantou/{obj_name}/light21.txt'  # 相机坐标系下的光源坐标
        ...
        t1 = time.time()
        rho, N , z= self.main_loop(imgs, K, light_p, light_d, z0_,zeta_vec, mask0,mu=0, imgs_save_path=imgs_save_path)
        t2 = time.time()
...
if __name__ == "__main__":
    ...
    # |----mask.png
    dep=np.load(f'testData/0909_50_zhuantou/side2_HR/depth_600.npy')
    Semi_calib('data/config_hzj.json').run(dep)
