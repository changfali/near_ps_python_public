...
class Semi_calib(object):
    ...
    def main_loop(self, images, K, light_p, light_d, z0, zeta_vec=None,mask0=None, mu=1, imgs_save_path=None):
        ...
        return (rho[:,:,[0,1,2]]/np.max(rho)*65535).astype(np.uint16), N 
    ...
    def run(self):
        ...
        mask_path = f'testData/one_stone/{obj_name}/mask.png'     
        K_path = f'testData/one_stone/{obj_name}/camera.mat'            # 相机内参
        lights_path = f'testData/one_stone/{obj_name}/light.mat'        # 相机坐标系下的光源坐标
        imgs_path = f'testData/one_stone/{obj_name}/imgs/'              # 图片文件夹
        imgs_save_path = f'testData/one_stone/{obj_name}/out/'                         # 结果保存路径
        depth_path = f'testData/one_stone/{obj_name}/depth.npy'
        ...
        imgs_dir = os.listdir(imgs_path) ; imgs_dir.sort(key = lambda x: int(x[12:-7]))     # 注意图片名称
        ...
        if self.use_depth:
            z0_ = np.load(depth_path)
            z0_ = z0_[::size,::size]
        else:
            z0_ = np.ones((imgs.shape[1]//2,imgs.shape[2]//2), np.float32)*z0


        # index = np.array([i for i in range(64)])
        # # index = [4,5,6,7,8,10,20,21,22,23,24,25,26,36,37,38,39,40,41,42,52,53,54,55,56,57,58]
        # # index = np.random.choice(128, 64, replace=False)
        # if 9 in index:
        #     index = np.delete(index, np.where(index == 9))
        # index = np.sort(index)
        # light_p = light_p[index,:]
        # light_d = light_d[index,:]
        # imgs = imgs[index,:,:,:]
        ...
        t1 = time.time()
        rho, N= self.main_loop(imgs, K, light_p, light_d, z0_,zeta_vec, mask0,mu=0, imgs_save_path=imgs_save_path)
        t2 = time.time()
...
if __name__ == "__main__":
    ...
    # |----imgs
    # |----out
    # |----camera.mat:使用['K']访问，相机内参
    # |----depth.npy
    # |----light.mat:使用['S']访问，相机坐标系下的光源位置
    # |----mask.png
    Semi_calib('data/config_hzj.json').run()
