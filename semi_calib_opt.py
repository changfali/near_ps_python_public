...
class Semi_calib(object):
    ...
    def make_gradient(self,mask):
        ...
        no_bottom = index_matrix[xc, yc][np.where(index_matrix[xc, yc] != 0)]
        print(no_bottom.shape, Dy.shape, Dym.shape, np.max(no_bottom), np.min(no_bottom))
        Dy[no_bottom,:] = Dym[no_bottom,:]
        Dx = Dxp
        [xc, yc], no_right =  self.rowfind(1 - Omega[:, :, 2], 1)
        no_right = index_matrix[xc, yc][np.where(index_matrix[xc, yc] != 0)]
    ...
    def main_loop(self, images, K, light_p, light_d, z0_, mask0=None, mu=1, imgs_save_path=None):
        '''
        input:
        ...
        if mask0 is None:
            mask0 = np.ones((h, w))
        z0 = np.ones_like(mask0, np.float32)*z0_
        #######
        ...
        for ratio in scales_list:
            ...
            z0 = cv2.resize(z0_ref, (w_ref//ratio+1, h_ref//ratio+1))
        ...
        Phi = Phi*max_I
        print('Max Height:', np.max(z_final)-np.min(z_final), np.max(z_final))
...
if __name__ == "__main__":
    ...
    lights_path = 'testData/spoon/light.mat'        # 相机坐标系下的光源坐标
    imgs_path = 'testData/spoon/imgs/'              # 图片文件夹
    imgs_save_path = 'out/'                         # 结果保存路径
    save_current_result = False                     # 是否保存中间结果
    cos4a=True                                      # 原始图像是否进行cos4a的矫正

    semi_n = Semi_calib('data/config.json')
    ...
    imgs_dir = os.listdir(imgs_path); imgs_dir.sort(key = lambda x: int(x[4:8]))      # 注意图片名称
